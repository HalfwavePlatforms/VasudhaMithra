...
export default function TopBar({
  ...
  return (
    <header className="h-16 bg-[var(--color-bg-primary)] border-b border-[var(--color-border)] flex items-center justify-between px-3.5 sm:px-8 sticky top-0 z-20">
      ...
      <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
        {onToggleSidebar && (
          <button
            ...
            className="p-2 -ml-1 text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-bg-secondary)] rounded-xl lg:hidden cursor-pointer shrink-0 transition-colors"
            title="Open Menu"
          >
        ...
        <div className="flex flex-col min-w-0">
          <span className="text-[9px] sm:text-[10px] font-semibold tracking-wider text-[var(--color-text-muted)] uppercase truncate">
            {breadcrumb || t("topbar.breadcrumbState")}
          </span>
          <h2 className="text-sm sm:text-base font-bold text-[var(--color-text-primary)] tracking-tight truncate">
            {title}
          </h2>
        </div>
      </div>

      {/* Global Search Bar (Hidden on phone, clean width on desktop/tablet) */}
      <div className="hidden md:flex relative w-52 lg:w-80 max-w-md mx-2">
        ...
        <input
          ...
          className="w-full pl-9 pr-10 py-1.5 text-xs bg-[var(--color-bg-secondary)] border border-[var(--color-border-strong)] rounded-lg text-[var(--color-text-primary)] placeholder-[var(--color-text-muted)] focus:outline-none focus:ring-1 focus:ring-[var(--color-accent)] focus:border-[var(--color-accent)] transition-all shadow-xs"
        />
        <div className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none">
          <kbd className="px-1 py-0.5 text-[9px] font-mono text-[var(--color-text-muted)] bg-[var(--color-bg-tertiary)] border border-[var(--color-border-strong)] rounded">
            ⌘K
          </kbd>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-4 shrink-0">
        ...
        <div className="relative" ref={langRef}>
          <button
            ...
            className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1.5 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] text-xs font-semibold text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:border-[var(--color-border-strong)] transition-all shadow-xs cursor-pointer"
            title={t("topbar.selectLanguage")}
          >
            <Globe className="w-3.5 h-3.5 text-[var(--color-accent)] shrink-0" />
            <span className="tracking-wide hidden sm:inline">{currentLang.nativeName}</span>
            <span className="tracking-wide sm:hidden uppercase text-[10px] font-bold">{currentLang.code}</span>
